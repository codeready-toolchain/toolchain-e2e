...
setup-toolchainclusters: ksctl
	...
	if [[ ${SECOND_MEMBER_MODE} == true ]]; then ${BIN_DIR}/ksctl adm register-member --host-ns "$(HOST_NS)" --member-ns "$(MEMBER_NS_2)" --host-kubeconfig "$(or ${KUBECONFIG}, ${HOME}/.kube/config)" --member-kubeconfig "$(or ${KUBECONFIG}, ${HOME}/.kube/config)" ${LETS_ENCRYPT_PARAM} -y ; fi

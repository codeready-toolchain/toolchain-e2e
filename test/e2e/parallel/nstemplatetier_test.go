...
func verifyResourceUpdatesForUserSignups(t *testing.T, hostAwait *wait.HostAwaitility, memberAwaitility *wait.MemberAwaitility, userSignups []*toolchainv1alpha1.UserSignup, tier *tiers.CustomNSTemplateTier) {
	...
	for _, usersignup := range userSignups {
		...
		nsTemplateSet, err := memberAwaitility.WaitForNSTmplSet(t, usersignup.Status.CompliantUsername, wait.UntilNSTemplateSetHasTier(tier.Name), tiers.UntilNSTemplateSetHasStatusTemplateRefs(tiers.GetTemplateRefs(t, hostAwait, tier.Name)))
		if err != nil {

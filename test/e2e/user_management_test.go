...
func (s *userManagementTestSuite) SetupSuite() {
	userAccountList := &v1alpha1.UserAccountList{}
	s.testCtx, s.awaitility = testsupport.WaitForDeployments(s.T(), userAccountList)
...
func (s *userManagementTestSuite) TestUserDisabled() {
	...
	userSignup, err = s.awaitility.Host().WaitForUserSignup(userSignup.Name, wait.UntilUserSignupHasConditions(approvedByAdmin()...))
	require.NoError(s.T(), err)
	...
	// Tina TODO: Wait for this instead
	...
	err = s.awaitility.Client.Update(context.TODO(), userAccount)
	require.NoError(s.T(), err)

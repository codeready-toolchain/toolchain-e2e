...
func (s *userSignupIntegrationTest) TestAutomaticApproval() {
	...
	s.T().Run("set low max number of users and expect that user won't be approved nor provisioned but added on waiting list", func(t *testing.T) {
		...
		s.hostAwait.UpdateToolchainConfig(test.AutomaticApproval().Enabled().MaxUsersNumber(originalMursPerDomainCount["internal"] + originalMursPerDomainCount["external"]))
		...
		t.Run("increment the max number of users and expect the first unapproved user will be provisioned", func(t *testing.T) {
			// when
			s.hostAwait.UpdateToolchainConfig(test.AutomaticApproval().Enabled().MaxUsersNumber(originalMursPerDomainCount["internal"] + originalMursPerDomainCount["external"] + 1))

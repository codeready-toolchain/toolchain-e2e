...
type CustomNSTemplateTier struct {
	...
	ClusterResourcesTierName string
	...
	NamespaceResourcesTierName string
}
...
func WithClusterResources(otherTier *toolchainv1alpha1.NSTemplateTier) CustomNSTemplateTierModifier {
	return func(hostAwait *HostAwaitility, tier *CustomNSTemplateTier) error {
		tier.ClusterResourcesTierName = otherTier.Name
		tier.Spec.ClusterResources = otherTier.Spec.ClusterResources
		tmplRef, err := duplicateTierTemplate(hostAwait, tier.Name, otherTier.Spec.ClusterResources.TemplateRef, otherTier.Namespace)
		if err != nil {
			return err
		}
...
func WithNamespaceResources(otherTier *toolchainv1alpha1.NSTemplateTier) CustomNSTemplateTierModifier {
	return func(hostAwait *HostAwaitility, tier *CustomNSTemplateTier) error {
		tier.NamespaceResourcesTierName = otherTier.Name
		tier.Spec.Namespaces = otherTier.Spec.Namespaces
		// namespaces and namespace-scoped resources
		for i, ns := range tier.Spec.Namespaces {
			tmplRef, err := duplicateTierTemplate(hostAwait, tier.Name, ns.TemplateRef, otherTier.Namespace)
			if err != nil {
				return err
			}
...
func duplicateTierTemplate(hostAwait *HostAwaitility, tierName, origTemplateRef, namespace string) (string, error) {

...
func TestGetMetricValue(t *testing.T) {
	...
	t.Run("valid metrics", func(t *testing.T) {
		t.Run("counter with no labels", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "sandbox_user_signups_total", []string{})
			// then
		...
		t.Run("counter with single label", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "workqueue_depth", []string{"name", "masteruserrecord-controller"})
			// then
			require.NoError(t, err)
		...
		t.Run("counter with two labels", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "controller_runtime_reconcile_total", []string{"controller", "usersignup-controller", "result", "success"})
			// then
			require.NoError(t, err)
		...
		t.Run("gauge with no labels", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "sandbox_master_user_record_current", []string{})
			// then
			require.NoError(t, err)
			assert.Equal(t, float64(7), result)
		})
	...
	t.Run("failures", func(t *testing.T) {
		t.Run("metric does not exist", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "non_existent_counter", []string{})
			// then
			require.Error(t, err)
		...
		t.Run("metric family exists but labels do not match", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "workqueue_depth", []string{"name", "non-existent-controller"})
			// then
			require.Error(t, err)
		...
		t.Run("odd number of label parameters", func(t *testing.T) {
			// when
			result, err := getMetricValue(nil, url, "workqueue_depth", []string{"name"})
			// then
			require.Error(t, err)

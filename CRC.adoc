...
== Step-by-step guide — OpenShift local (formerly CRC, CodeReady Containers)
...
While your local OpenShift cluster boots, you can go ahead and prepare the Quay repositories and the local ones to be
able to run the tests.

=== Prepare the projects
==== Creating the Quay repositories and making them public

NOTE: Instead of creating the repositories yourself, you can also run the `e2e-run` target with Make as follows, which
will create the repositories for you: `make e2e-run`. Bear in mind, though, that the first run is very likely to fail
because it is hard to make all these repositories public by hand while the tests are running. The second run should run
with no issues.
